...
TGB_DATA_NAME = ["tgbl-wiki","tgbl-review", "tgbl-coin", "tgbl-flight", "tgbl-comment", "thgl-software", "thgl-myket", "thgl-github", "thgl-forum"]
...
class Data:
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, edge_ids: np.ndarray, labels: np.ndarray, neg_samples: np.ndarray = None, split=None ):
        ...
        if neg_samples is not None:
            ...
            neg_samples_len = neg_samples.shape[0] if hasattr(neg_samples, 'shape') else len(neg_samples)
            if isinstance(neg_samples, np.ndarray) and neg_samples.ndim > 1:
                self.num_neg_samples = neg_samples.shape[1]
            else:
                self.num_neg_samples = max(len(np.asarray(neg_row).reshape(-1)) for neg_row in neg_samples)
            # split-aware negative samples may correspond to val-only, test-only, or the full slice.
            if split is not None:
                valid_neg_sample_counts = {
                    len(src_node_ids),
                    np.where(split == 1)[0].shape[0],
                    np.where(split == 2)[0].shape[0],
                }
                assert neg_samples_len in valid_neg_sample_counts, 'Number of negative samples does not match the provided evaluation split!'
            else:
                assert len(src_node_ids) == neg_samples_len, 'Number of negative samples does not match the number of test samples!'
        else:
...
def get_link_prediction_data(dataset_name: str, val_ratio: float = 0.15, test_ratio: float = 0.15, dataset_path='./processed_data/', use_edge_feat=False, use_node_feat=False, logger=None, model_name: str = None):
    """
    ...
    """
    pad_features_for_model = model_name != 'TPNet'
    ...
    if dataset_name in TGB_Seq_DATA_NAME:
        ...
        edge_types = data['edge_type'].astype(np.longlong) if dataset_name.startswith('thgl-') and 'edge_type' in data else None
        ...
        def load_split_ns(split_name: str, split_mask: np.ndarray):
            ns_path = os.path.join(dataset_path, processed_dataset_name, f"{processed_dataset_name}_{split_name}_ns.npy")
            if os.path.exists(ns_path):
                return np.load(ns_path, allow_pickle=True)
            ...
            split_ns_dict = dataset.ns_sampler.eval_set[split_name]
            split_ns = []
            if edge_types is None:
                iterator = zip(src_node_ids[split_mask], dst_node_ids[split_mask], node_interact_times[split_mask])
            else:
                iterator = zip(src_node_ids[split_mask], dst_node_ids[split_mask], node_interact_times[split_mask], edge_types[split_mask])
            for sample in iterator:
                if edge_types is None:
                    src, dst, time = sample
                    lookup_keys = [(src, dst, time)]
                else:
                    src, dst, time, edge_type = sample
                    lookup_keys = [(time, src, edge_type)]
                for lookup_key in lookup_keys:
                    if lookup_key in split_ns_dict:
                        tmp_negs = split_ns_dict[lookup_key]
                        break
                else:
                    raise KeyError(f"No such {split_name} sample")

                if num_negs == 0:
                    num_negs = len(tmp_negs)
                else:
                    assert num_negs == len(tmp_negs), f"Number of negative samples is not consistent! {num_negs} != {len(tmp_negs)}"
                split_ns.append(tmp_negs)

            try:
                split_ns = np.array(split_ns)
            except ValueError:
                split_ns = np.array(split_ns, dtype=object)
            np.save(ns_path, split_ns)
            return split_ns

        val_ns = load_split_ns('val', val_mask)
        test_ns = load_split_ns('test', test_mask)
        ...
        if not use_node_feat:
            ...
            if pad_features_for_model and node_raw_features.shape[1] < MAX_FEAT_DIM:
        ...
        if not use_edge_feat:
            ...
            if pad_features_for_model and edge_raw_features.shape[1] < MAX_FEAT_DIM:
        ...
        val_data = Data(src_node_ids=src_node_ids[val_mask], dst_node_ids=dst_node_ids[val_mask],
                        node_interact_times=node_interact_times[val_mask], edge_ids=edge_ids[val_mask], labels=labels[val_mask], neg_samples=val_ns, split=val_split)
        test_split = np.full(test_mask.sum(),2)
