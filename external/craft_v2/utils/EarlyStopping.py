...
class EarlyStopping(object):
    ...
    def step(self, metrics: list, model: nn.Module = None):
        """
        ...
        for metric_tuple in metrics:
            ...
            if metric_value is None or not math.isfinite(float(metric_value)):
                self.logger.warning(f"skip checkpoint update because {metric_name} is non-finite: {metric_value}")
    ...
    def save_checkpoint(self, model: nn.Module):
        ...
        # if self.model_name in ['JODIE', 'DyRep', 'TGN']:
        #     torch.save(model[0].memory_bank.node_raw_messages.cpu(), self.save_model_nonparametric_data_path)
    ...
    def load_checkpoint(self, model: nn.Module, map_location: str = None):
        ...
        if os.path.exists(self.save_model_path):
            ...
            self.logger.error(f"no model found at {self.save_model_path}, re-training from scratch")
