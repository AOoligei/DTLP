...
TPNET_TRAINING_MODE_ALIASES = {
    'jr': 'joint_random',
    'joint_random': 'joint_random',
    'jh': 'joint_hard',
    'joint_hard': 'joint_hard',
    'dc': 'decoupled',
    'decoupled': 'decoupled',
    'sg': 'decoupled_stopgrad',
    'decoupled_stopgrad': 'decoupled_stopgrad',
}


def normalize_tpnet_training_mode(training_mode: str) -> str:
    if training_mode not in TPNET_TRAINING_MODE_ALIASES:
        valid_modes = ', '.join(sorted(TPNET_TRAINING_MODE_ALIASES.keys()))
        raise ValueError(f"Unsupported training_mode {training_mode}! Valid choices: {valid_modes}")
    return TPNET_TRAINING_MODE_ALIASES[training_mode]
...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--training_mode', type=str, default='sg', choices=sorted(TPNET_TRAINING_MODE_ALIASES.keys()),
                        help='PAS training mode: jr=joint random, jh=joint hard, dc=decoupled, sg=decoupled stop-grad (our method)')
    parser.add_argument('--lambda_pair', type=float, default=1.0, help='weight for PAS pair-path loss')
    parser.add_argument('--lambda_orth', type=float, default=0.0, help='weight for PAS orthogonality regularization')
    parser.add_argument('--rho_ema_momentum', type=float, default=0.95, help='EMA momentum for PAS recurrence-rate tracking')
    parser.add_argument('--tpnet_dim_factor', type=int, default=10, help='TPNet random projection dimension factor')
    parser.add_argument('--tpnet_num_layer', type=int, default=2, help='TPNet random walk depth')
    parser.add_argument('--tpnet_time_decay', type=float, default=1e-6, help='TPNet random projection time decay weight')
    try:
    ...
    args.training_mode = normalize_tpnet_training_mode(args.training_mode)
    args.use_pas = args.model_name == 'TPNet' or (args.model_name == 'CRAFT' and '--training_mode' in sys.argv[1:])
    if args.model_name == 'CRAFT' and args.use_pas:
        args.loss = 'BCE'
...
def load_link_prediction_best_configs(args: argparse.Namespace):
    ...
    if args.model_name == 'TGAT':
        ...
        if args.dataset_name in ['reddit', 'UNvote']:
            ...
            args.dropout = 0.1
    elif args.model_name == 'CRAFT':
        # Per CRAFT paper Table 6 (appendix): official per-dataset hyperparameters.
        args.sample_neighbor_strategy = 'recent'
        args.shuffle = True
        args.loss = 'BPR'
        ...
        args.use_pos = True
        args.use_mrr_val = args.dataset_name.startswith(('tgbl-', 'tkgl-', 'thgl-'))
        args.output_cat_repeat_times = args.dataset_name in ['mooc']

        # CRAFT Table 6 per-dataset hyperparameters (nn, hid_drop, attn_drop, emb_drop, layers, bs, F)
        _CRAFT_T6 = {
            ...
            'GoogleLocal': (60,  0.2, 0.1, 0.2, 2, 400, 128),
        ...
        if args.dataset_name in _CRAFT_T6:
            nn, hd, ad, ed, nl, bs, F = _CRAFT_T6[args.dataset_name]
            args.num_neighbors = nn
            args.hidden_dropout = hd
            args.attn_dropout_prob = ad
            args.emb_dropout_prob = ed
            args.num_layers = nl
            args.batch_size = bs
            args.embedding_size = F
            args.dropout = hd  # alias for legacy use
        else:
            # Datasets not in Table 6 (e.g., tkgl-*, thgl-*): conservative defaults
            args.num_neighbors = 60
            args.num_layers = 1
            args.dropout = 0.2
            args.hidden_dropout = 0.2
            args.attn_dropout_prob = 0.2
            args.emb_dropout_prob = 0.2
            args.batch_size = 200
        ...
        args.sample_neighbor_strategy = 'recent'
        args.dropout = 0.1
        args.use_mrr_val = True
        args.tpnet_dim_factor = 10
        # Per TPNet official (/home/chenlibin/TPNet/utils/load_configs.py):
        # - DyGLib datasets use rp_num_layer=3
        # - TGB datasets (tgbl-*) use rp_num_layer=2 (per TGB_TPNet)
        if args.dataset_name.startswith('tgbl-') or args.dataset_name.startswith('tkgl-') or args.dataset_name.startswith('thgl-'):
            args.tpnet_num_layer = 2
        else:
            args.tpnet_num_layer = 3
        # Per TPNet official best_configs time_decay:
        if args.dataset_name in ['tgbl-review', 'lastfm', 'uci']:
            args.tpnet_time_decay = 1e-7
        elif args.dataset_name in ['mooc']:
            args.tpnet_time_decay = 1e-5
        else:
            args.tpnet_time_decay = 1e-6
    else:
