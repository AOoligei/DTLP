...
class CRAFT(torch.nn.Module):
    ...
    def __init__(self, n_layers, n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps, initializer_range, n_nodes, max_seq_length, device, loss_type, use_pos=True, input_cat_time_intervals=False, output_cat_time_intervals=True, output_cat_repeat_times=False, num_output_layer=1,  emb_dropout_prob=0.1, skip_connection=False):
        ...
        if self.output_cat_repeat_times:
            output_dim += self.hidden_size
        self.backbone_output_dim = output_dim
    ...
    def compute_candidate_embeddings(self, src_neighb_seq, src_neighb_seq_len, neighbors_interact_times, cur_times, test_dst = None, dst_last_update_times = None, return_cross_attention_output: bool = False):
        bs = src_neighb_seq.shape[0]
        src_neighb_seq_len = src_neighb_seq_len.clone()
        ...
        if return_cross_attention_output:
            return output
        if self.output_cat_time_intervals:
            if output is None:
    ...
    def forward(self, src_neighb_seq, src_neighb_seq_len, neighbors_interact_times, cur_times, test_dst = None, dst_last_update_times = None):
        output = self.compute_candidate_embeddings(src_neighb_seq=src_neighb_seq,
                                                   src_neighb_seq_len=src_neighb_seq_len,
                                                   neighbors_interact_times=neighbors_interact_times,
                                                   cur_times=cur_times,
                                                   test_dst=test_dst,
    ...
    def get_pre_final_embeddings(self, src_neighb_seq, src_neighb_seq_len, src_neighb_interact_times, cur_pred_times, test_dst, dst_last_update_times):
        ...
        return self.compute_candidate_embeddings(
            src_neighb_seq=src_neighb_seq,
            src_neighb_seq_len=src_neighb_seq_len,
            neighbors_interact_times=src_neighb_interact_times,
            cur_times=cur_pred_times.to(self.device),
            test_dst=test_dst,
            dst_last_update_times=dst_last_update_times.to(self.device),
            return_cross_attention_output=True,
        )

    def compute_src_dst_edge_embeddings(self, src_neighb_seq, src_neighb_seq_len, src_neighb_interact_times, cur_pred_times, test_dst, dst_last_update_times):
        edge_embeddings = self.get_pre_final_embeddings(
            src_neighb_seq=src_neighb_seq,
            src_neighb_seq_len=src_neighb_seq_len,
            src_neighb_interact_times=src_neighb_interact_times,
            cur_pred_times=cur_pred_times,
            test_dst=test_dst,
            dst_last_update_times=dst_last_update_times,
        )
    ...
    def predict(self, src_neighb_seq, src_neighb_seq_len, src_neighb_interact_times, cur_pred_times, test_dst, dst_last_update_times):
        ...
        src_neighb_interact_times = src_neighb_interact_times.to(self.device)
        src_neighb_seq_len = src_neighb_seq_len.to(self.device)
        candidate_embeddings = self.compute_candidate_embeddings(src_neighb_seq=src_neighb_seq,
                                                                 src_neighb_seq_len=src_neighb_seq_len,
                                                                 neighbors_interact_times=src_neighb_interact_times,
                                                                 cur_times=cur_pred_times.to(self.device),
                                                                 test_dst=test_dst,
                                                                 dst_last_update_times=dst_last_update_times.to(self.device))
        logits = self.output_layer(candidate_embeddings.view(-1, candidate_embeddings.shape[-1])).view(candidate_embeddings.shape[0], candidate_embeddings.shape[1], -1)
    ...
    def get_candidate_embeddings(self, src_neighb_seq, src_neighb_seq_len, src_neighb_interact_times, cur_pred_times, test_dst, dst_last_update_times):
        src_neighb_seq = src_neighb_seq.to(self.device) - self.dst_min_idx + 1
        test_dst = test_dst.to(self.device) - self.dst_min_idx + 1
        src_neighb_seq[src_neighb_seq < 0] = 0
        src_neighb_interact_times = src_neighb_interact_times.to(self.device)
        src_neighb_seq_len = src_neighb_seq_len.to(self.device)
        return self.compute_candidate_embeddings(src_neighb_seq=src_neighb_seq,
                                                 src_neighb_seq_len=src_neighb_seq_len,
                                                 neighbors_interact_times=src_neighb_interact_times,
                                                 cur_times=cur_pred_times.to(self.device),
                                                 test_dst=test_dst,
