...
class RandomProjectionModule(nn.Module):
    ...
    def clear_dynamic_state(self):
        """
        Clear only the accumulated temporal walk matrices (layers 1..num_layer) and time
        tracker. Layer 0 (the fixed random base map) is preserved so that replay or epoch
        rollover does not change the base random projection — keeping backup/restore and
        checkpointed states consistent.
        """
        for i in range(1, self.num_layer + 1):
            nn.init.zeros_(self.random_projections[i])
        self.now_time.data = self.begging_time.clone()
...
class TPNetEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, src_node_ids: np.ndarray,
        ...
        # mask the pad nodes (i.e., id = 0). NOTE: masked_fill returns a new tensor;
        # we must assign the result (in-place masked_fill_ would also work).
        embeddings = embeddings.masked_fill(torch.from_numpy(neighbor_node_ids == 0)[:, :, None].to(device), 0)
